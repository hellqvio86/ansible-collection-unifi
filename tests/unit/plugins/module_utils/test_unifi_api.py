...
def test_unifi_api_login_success():
    ...
    mock_info = {
        'status': 200,
        'set-cookie': f'TOKEN={jwt_token}; Path=/; HttpOnly'
    }
    
    with patch('ansible_collections.hellqvio86.unifi.plugins.module_utils.unifi_api.fetch_url', return_value=(mock_response, mock_info)):
        api.login()
...
def test_unifi_api_request():
    ...
    with patch('ansible_collections.hellqvio86.unifi.plugins.module_utils.unifi_api.fetch_url', return_value=(mock_response, mock_info)) as mock_fetch:
