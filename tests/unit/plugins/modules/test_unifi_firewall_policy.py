...
def test_firewall_policy_create():
    ...
    with patch('ansible_collections.hellqvio86.unifi.plugins.modules.unifi_firewall_policy.AnsibleModule') as mock_module_class, \
...
def test_firewall_policy_absent():
    ...
    with patch('ansible_collections.hellqvio86.unifi.plugins.modules.unifi_firewall_policy.AnsibleModule') as mock_module_class, \
        ...
        mock_api.request.side_effect = [
            ...
            ([{'name': 'Existing Policy', '_id': 'old123', 'source': {'zone_id': 'zone123'}, 'destination': {'zone_id': 'zone123'}}], {'status': 200}),
            # Delete policy
            (None, {'status': 204})
        ]
