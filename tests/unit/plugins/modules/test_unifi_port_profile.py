...
def test_port_profile_no_change():
    ...
    with (
        ...
        # Mock responses
        mock_api.request.side_effect = [
            # 1. Fetch existing profiles (matches desired)
            (
                [
                    {
                        "name": "Existing Profile",
...
def test_port_profile_duplicate_fails():
    params = {
        "host": "192.0.2.1",
        "username": "admin",
        "password": "password",
    ...
    with (
        ...
        mock_api.request.side_effect = [
            (
                [
                    {"_id": "p1", "name": "Duplicate Profile"},
                    {"_id": "p2", "name": "Duplicate Profile"},
                ],
                {"status": 200},
            ),
        ]
